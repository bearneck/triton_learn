...
DNN 所带来的计算机遇与 GPU 编程的实际难度之间的张力，催生了学界和产业界对领域特定语言（DSL）和编译器的大量关注。然而，无论是基于多面体机制（如 Tiramisu [BAGHDADI2021]_、Tensor Comprehensions [VASILACHE2018]_）还是调度语言（如 Halide [JRK2013]_、TVM [CHEN2018]_）的系统，在灵活性上仍不及手写的计算 kernel，且速度也明显慢于 `cuBLAS <https://docs.nvidia.com/cuda/cublas/index.html>`_、`cuDNN <https://docs.nvidia.com/deeplearning/cudnn/api/index.html>`_ 等库中最优的手写实现。

本项目的核心前提：基于块算法 [LAM1991]_ 的编程范式能够促进神经网络高性能计算 kernel 的构建。我们重新审视了 GPU 的"单程序多数据"（SPMD [AUGUIN1983]_）执行模型，并提出一种变体——以程序（program）而非线程（thread）为单位进行分块。

以矩阵乘法为例，CUDA 与 Triton 的差异如下：
...
.. image:: cuda-parallel-matmul.png
   :width: 80%
   :align: center
...
.. image:: triton-parallel-matmul.png
   :width: 80%
   :align: center
...
我们所提出范式面临的主要挑战在于工作调度问题，即每个程序实例的工作应如何划分，才能在现代 GPU 上高效执行。为此，Triton 编译器大量使用了 **块级数据流分析** 技术——基于目标程序的控制流与数据流结构，对迭代块进行静态调度的方法。所得系统的实际效果出人意料地好：编译器能够自动应用一系列有趣的优化（例如自动内存合并、线程 swizzling、预取、自动向量化、tensor core 感知的指令选择、共享内存分配与同步、异步拷贝调度）。
