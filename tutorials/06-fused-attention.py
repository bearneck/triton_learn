...
这是 Tri Dao（https://tridao.me/publications/flash2/flash2.pdf）提出的
Flash Attention v2 算法的 Triton 实现。
...
def _attn_fwd_inner(acc, l_i, m_i, q,  #
    ...
    for start_n in tl.range(lo, hi, BLOCK_N, warp_specialize=warp_specialize):
        ...
        p = p.to(dtype)
        # 注意：FP8 的非转置 v 仅在 Blackwell 上支持
        acc = tl.dot(p, v, acc)
        # 更新 m_i 和 l_i
        # 将此操作放在循环末尾以降低寄存器压力
        l_i = l_i * alpha + l_ij
...
def prune_invalid_configs(configs, named_args, **kwargs):
    ...
    # 过滤 BLOCK_M > N_CTX 的配置
    # 当 causal 为 True 时，过滤 BLOCK_M < BLOCK_N 的配置
    return [
...
def _attn_fwd(sm_scale, M,  #
    ...
    # 阶段 1：带外（off-band）
    # 当 causal = True 时，STAGE = 3，_attn_fwd_inner 接收 STAGE = 1
    # 当 causal = False 时，STAGE = 1，_attn_fwd_inner 接收 STAGE = 3
    if STAGE & 1:
...
def _attn_bwd_dkdv(dk, dv,  #
    ...
    # BLOCK_N1 必须是 BLOCK_M1 的整数倍，否则代码无法正常运行。
...
def _attn_bwd_dq(dq, q, K, V,  #
    ...
    # D（= delta）已预先除以 ds_scale。
    Di = tl.load(D + offs_m)
    # BLOCK_M2 必须是 BLOCK_N2 的整数倍，否则代码无法正常运行。
    ...
    for blk_idx in range(num_steps):
        ...
        ds = ds.to(tl.float16)
        # 计算 dQ。
        # 注意：最终需要对 dq 进行反缩放，因为 kT 已经过预缩放。
...
def _attn_bwd(Q, K, V, sm_scale,  #
    ...
    if CAUSAL:
        # 为掩码（对角线）block 计算 dQ。
        # 注意：以下代码从右到左逐行扫描 QK^T（但在每次调用 _attn_bwd_dq 内部，
        # 是从左到右扫描），这并非出于什么重要原因，
        # 只是为了尽量复用上面计算 dK 和 dV 的循环结构。
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, sm_scale, warp_specialize=True):
        ...
        # 对于 Hopper + warpspec，使用 device_descriptor。
        if supports_host_descriptor() and not (is_hopper() and warp_specialize):
            # 注意：在 Hopper 上，无法对非转置的第二个张量执行 FP8 点积
...
TORCH_HAS_FP8 = hasattr(torch, 'float8_e5m2')
BATCH, N_HEADS = 4, 32
# 固定 head 和 batch=4，变化序列长度
configs = []
for HEAD_DIM in [64, 128]:
    for mode in ["fwd", "bwd"]:
        for causal in [True, False]:
            # 在 Hopper 上为 causal fwd 启用 warpspec
...
def bench_flash_attention(BATCH, H, N_CTX, HEAD_DIM, causal, warp_specialize, mode, provider, device=DEVICE):
    ...
    if causal:
        total_flops *= 0.5
    if mode == "bwd":
        total_flops *= 2.5  # 2.0（反向）+ 0.5（重计算）
    return total_flops * 1e-12 / (ms * 1e-3)


if __name__ == "__main__":
    # 目前仅在 Ampere 之后的 GPU 上有效
