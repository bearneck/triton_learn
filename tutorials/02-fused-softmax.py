...
def naive_softmax(x):
    ...
    # 合计：读取 5MN + 2M 个元素；写入 3MN + 2M 个元素
    return ret
...
def softmax_kernel(output_ptr, input_ptr, input_row_stride, output_row_stride, n_rows, n_cols, BLOCK_SIZE: tl.constexpr,
    ...
    for row_idx in tl.range(row_start, n_rows, row_step, num_stages=num_stages):
        # stride（步长）表示指针前进一行所需增加的量
        row_start_ptr = input_ptr + row_idx * input_row_stride
        # block size 是大于 n_cols 的最小 2 的幂次，使得每行恰好可放入一个 block
        col_offsets = tl.arange(0, BLOCK_SIZE)
        input_ptrs = row_start_ptr + col_offsets
        # 将该行加载到 SRAM（片上内存），由于 BLOCK_SIZE 可能大于 n_cols，使用掩码进行保护
        mask = col_offsets < n_cols
        row = tl.load(input_ptrs, mask=mask, other=-float('inf'))
        # 减去最大值以保证数值稳定性
        row_minus_max = row - tl.max(row, axis=0)
        # 注意：Triton 中的指数运算速度快但精度为近似值（类似于 CUDA 中的 __expf）
...
def softmax(x):
    ...
    # 每次循环迭代的 block size 为大于 x 列数的最小 2 的幂次
    BLOCK_SIZE = triton.next_power_of_2(n_cols)

    # 另一个技巧是通过增加每行分配的 warp（线程束）数量（`num_warps`）
    # 来让编译器为每行使用更多线程。
    # 在下一个教程中，你将看到如何以更自然的方式自动调优该值，
    # 而无需手动设置启发式参数。
    num_warps = 8

    # 软件流水线（software pipelining）的阶段数。
    num_stages = 4 if SIZE_SMEM > 200000 else 2

    # 分配输出张量
    y = torch.empty_like(x)

    # 预编译 kernel 以获取寄存器使用量并计算线程占用率（thread occupancy）。
    ...
    # 创建若干个持久化 program（persistent programs）。
