...
def add_kernel(x_ptr,  # 指向第一个输入向量的 *指针*。
    ...
    # 有多个 'program'（程序实例）在处理不同的数据。在此标识当前是哪个 program：
    pid = tl.program_id(axis=0)  # 我们使用一维启动网格，因此 axis 为 0。
    # 该 program 将处理相对于初始数据有偏移量的输入。
    # 例如，若向量长度为 256，block_size 为 64，则各 program 分别访问
    # [0:64, 64:128, 128:192, 192:256] 的元素。
    # 注意 offsets（偏移量）是一组指针：
    block_start = pid * BLOCK_SIZE
    offsets = block_start + tl.arange(0, BLOCK_SIZE)
    # 创建掩码（mask），防止内存操作越界访问。
    mask = offsets < n_elements
    # 从 DRAM 加载 x 和 y，若输入大小不是 block size 的整数倍，则用掩码屏蔽多余元素。
...
def add(x: torch.Tensor, y: torch.Tensor):
    ...
    # SPMD 启动网格（launch grid）表示并行运行的 kernel 实例数量。
    # 类似于 CUDA 的启动网格，可以是 Tuple[int] 或 Callable(metaparameters) -> Tuple[int]。
    # 此处使用一维网格，大小等于 block 的数量：
    grid = lambda meta: (triton.cdiv(n_elements, meta['BLOCK_SIZE']), )
    # 注意：
    #  - 每个 torch.tensor 对象会被隐式转换为指向其首元素的指针。
    #  - 经 `triton.jit` 修饰的函数可通过启动网格索引，从而获得可调用的 GPU kernel。
    #  - 不要忘记以关键字参数的形式传递元参数（meta-parameters）。
    add_kernel[grid](x, y, output, n_elements, BLOCK_SIZE=1024)
    # 返回 z 的句柄；但由于尚未调用 `torch.cuda.synchronize()`，
    # 此时 kernel 仍在异步运行。
    return output
